  ...
  return (
    <div className="mt-3 space-y-3">
      <section className="border border-brass/60 bg-steel">
        <header className="border-b border-rule bg-plate px-3 py-2">
          <p className="text-[10px] tracking-[0.3em] text-brass uppercase">The era closes</p>
          <h2 className="mt-1 font-slab text-[24px] leading-none font-extrabold text-ink sm:text-[32px]">
            {winner ? `${winner.name} stands first` : "The books are shut"}
          </h2>
          <p className="mt-1 text-[11px] text-dim">
            {windows} window{windows === 1 ? "" : "s"} played to {winConditionLabel(state.game.winCondition)}
            {winner ? ` · ${formatMoney(winner.value)} at the close` : ""}
            {me?.isBankrupt ? " · your house was carried out by the court" : ""}
          </p>
        </header>
      ...
      <div className="grid items-start gap-3 lg:grid-cols-2">
        <Panel title="How the era was decided" aside={winConditionLabel(state.game.winCondition)}>
            ...
            {state.players.map((player) => (
              ...
              >
                ...
                <span className="text-[10px] text-faint">
                  {player.isBot ? "automated" : "human"} · {player.milestonesPassed.length} thresholds
                  crossed
                </span>
              </li>
            ))}
          ...
          <p className="pt-2 text-[10px] text-faint">
            Orders are closed at a finished table. The wire below stays open, and a seated house can
            open the next era at any time.
          </p>
        </Panel>
